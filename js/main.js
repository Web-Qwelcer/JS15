...
signup.addEventListener("click", (event) => {
  if (task2.Login.value.trim() == "" || task2.Pass.value.trim() == "" || task2.Email.value.trim() == "") {
    alert("Enter Data!!");
  }
  else if (task2.Pass.value.trim() < 8 && task2.Pass.value.trim() !== task2.Pass_confirm.value.trim()) {
...
save.addEventListener("click", (event) => {
  ...
  let inputData = person(task3.firstname.value, task3.lastname.value, task3.birthday.value, task3.gender.value, task3.country.value, task3.city.value, skillValue);
...
AddColor.addEventListener("click", () => {
  ...
  if (parseInt(task4.red.value) < 0 || parseInt(task4.red.value) > 255 || parseInt(task4.green.value) < 0 || parseInt(task4.green.value) > 255 || parseInt(task4.blue.value) < 0 || parseInt(task4.blue.value) > 255 || task4.red.value === "" || task4.green.value === "" || task4.blue.value === "" || task4.red.value.length > 3 || task4.green.value.length > 3 || task4.blue.value.length > 3) {
    alert("Wrong DATA");
  }
  else {
...
function colorCreate(red, green, blue) {
  (red = task4.red.value), (green = task4.green.value), (blue = task4.blue.value);
  ...
  $color.style.backgroundColor = `rgb(${parseInt(task4.red.value)}, ${parseInt(task4.green.value)}, ${parseInt(task4.blue.value)})`;
}
